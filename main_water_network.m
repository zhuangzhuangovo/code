...
junction_infoMatrix = epanetData.JUNCTIONS; % Junctions
tanks_infoMatrix    = epanetData.TANKS;     % Tanks
pipe_infoMatrix     = epanetData.PIPES;     % Pipes
reservoirs_infoMatrix = epanetData.RESERVOIRS; % Reservoirs
pump_infoMatrix     = epanetData.PUMPS;     % Pumps

%% ==================== Initialize Tank Cross-Section Areas ====================
csa = zeros(numel(tanks_infoMatrix),numel(tanks_infoMatrix));
for i = 1:numel(tanks_infoMatrix)
    diameter = tanks_infoMatrix(i).Diameter * 0.0254; % Convert to meters
    r = diameter / 2;                                  % Radius
...
convertToChar = @(id) num2str(id); % Convert numeric ID to string
...
for k = 1:numel(pump_ids)
    ...
    for m = 1:numel(junction_infoMatrix)
        if junction_infoMatrix(m).ID == raw_id
            node_order{current_idx} = node_id;
            index_map(node_id) = current_idx;
            current_idx = current_idx + 1;
            found = true;
...
n_res = size(epanetData.RESERVOIRS,1);
n_junc = size(epanetData.JUNCTIONS,1);
n_tank = size(epanetData.TANKS,1);
...
for k = 1:numel(pump_infoMatrix)
    ...
    i = index_map(from_id); j = index_map(to_id);
    lenPump = 100 * rand(1) * 0.3048;
    dPump = 10 * rand(1) * 0.0254;
    G = discretized(dPump, lenPump);
    A(i,i) = A(i,i) - G; A(i,j) = A(i,j) + G;
    A(j,j) = A(j,j) - G; A(j,i) = A(j,i) + G;
end

A(1:2,:) = 0; % Reservoir rows are constant
...
lambda = 1;
if rank([lambda*E-A, B]) == 4
    disp('System is R-controllable')
end
if det(lambda*E - A) == 0
    disp('System is regular')
end
...
% Algebraic & differential partitioning
N_c = E_c(q+1:end,q+1:end);
A_1 = A_c(1:q,1:q); B_1 = B_c(1:q,:);
B_2 = B_c(q+1:end,:);
...
u_bar = rand(m, T)*2-1;
z1 = 5*rand(q,1); % Differential initial state

y_bar = zeros(p,T);
x_bar = zeros(size(P,1),T);
...
%% ==================== Step 5: Identify System for ID+MPC ====================
i = 10;
u = u_bar(:,end-T+3:end);
y = y_bar(:,end-T+3:end);
...
cvx_clear; % Clear CVX environment before loop

t = 1;
while t < K-T+s
    ys = (t<=50)*[3;3] + (t>50)*[1;5];
    us = zeros(m,1);
